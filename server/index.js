...
passport.use(new Strategy(( email , password, cb) => {
  db.query('SELECT id, name, email, password FROM users WHERE email = ?', [email]).then(dbResults => {
...
app.get('/users', (req, res) => {
  db.query('SELECT id, name, email FROM users').then(results => {
    res.json(results);
  })
})
...
app.get('/users/:id',
        ...
        (req, res) => {
          db.query('SELECT id, name, email FROM users WHERE id = ?', [req.params.id]).then(results => {
            res.json(results);
          })
        });
